...
def plot_results(args, **kwargs):
    ...
    if 'va_topdown' in kwargs:
        ...
        for i in range(len(args.size_tab) - 1):
            ...
            #print('Number of time steps (index {}): {}'.format(i, wpf_hist[i].size(2)))

            #print(np.size(np.linspace(0, wpf_hist[i].size(2) - 1, wpf_hist[i].size(2))))
            #print(np.size(wpf_hist[i].mean(0).mean(1).cpu().numpy()))
